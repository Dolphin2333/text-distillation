...
##########################################################
def load_synthetic(path, num_classes=2):
...
##########################################################
def train_student(X_syn, y_syn, X_val, y_val, seed, epochs=200):
    torch.manual_seed(seed)
    np.random.seed(seed)

    model = TextMLP(
        d_in=768,
        d_hidden=256,   # ← make sure this matches your distillation MLP
        num_classes=2
    ).cuda()
...
def main():
    ...
    for path in args.paths:
        ...
        for seed in args.seeds:
            acc = train_student(X_syn, y_syn, X_val, y_val, seed)
            accs.append(acc)
