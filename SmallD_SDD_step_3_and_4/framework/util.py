...
class ImageIntervention(object):
    ...
    def do(self, x, seed):
        # If there is no augmentation key (e.g., strategy='none'), just return x
        if not self.not_single and (self.keys is None or len(self.keys) == 0):
            return x
...
# ======================================================================
# Boost-DD utility helpers (optional)
# ======================================================================

def concat_synthetic_blocks(old_data, old_labels, new_data, new_labels):
    """
    Concatenate previous-stage synthetic data with a new block and return
    the concatenated tensors plus the prefix length.

    Parameters
    ----------
    old_data : torch.Tensor or None
        Shape [N_old, D], or None if there is no previous stage.
    old_labels : torch.Tensor or None
        Shape [N_old, ...], or None.
    new_data : torch.Tensor
        Shape [N_new, D].
    new_labels : torch.Tensor or None
        Shape [N_new, ...], or None.
    ...
    data_all : torch.Tensor
        Shape [N_old + N_new, D].
    labels_all : torch.Tensor or None
        Shape [N_old + N_new, ...] if labels are provided, else None.
    old_prefix_size : int
        Number of rows in data_all that come from the previous stage (N_old).
    """
    if old_data is None:
        data_all = new_data.clone()
        labels_all = None if new_labels is None else new_labels.clone()
        old_prefix_size = 0
        return data_all, labels_all, old_prefix_size

    if old_data.dim() != 2 or new_data.dim() != 2:
        raise ValueError("concat_synthetic_blocks: old_data and new_data must be 2D tensors [N, D].")

    if old_data.shape[1] != new_data.shape[1]:
        raise ValueError(
            f"concat_synthetic_blocks: feature dimension mismatch: "
            f"old_data has D={old_data.shape[1]}, new_data has D={new_data.shape[1]}."
        )

    data_all = torch.cat([old_data, new_data], dim=0)
    old_prefix_size = old_data.shape[0]

    labels_all = None
    if old_labels is not None or new_labels is not None:
        if old_labels is None or new_labels is None:
            raise ValueError(
                "concat_synthetic_blocks: either both old_labels and new_labels must be provided, or none."
            )
        if old_labels.shape[1:] != new_labels.shape[1:]:
            raise ValueError(
                "concat_synthetic_blocks: label shapes after the first dimension do not match: "
                f"{old_labels.shape[1:]} vs {new_labels.shape[1:]}."
            )
        labels_all = torch.cat([old_labels, new_labels], dim=0)

    return data_all, labels_all, old_prefix_size


def save_synthetic_to_h5(path, data, labels=None, overwrite=False):
    """
    Save synthetic dataset to an HDF5 file with keys:
        - 'data' : float32 array [N, D]
        - 'label' : (optional) array with same first dimension
    ...
    path : str
        Target HDF5 path.
    data : torch.Tensor
        Synthetic data of shape [N, D].
    labels : torch.Tensor or None
        Optional labels of shape [N, ...].
    overwrite : bool
        If False and file exists, raise an error instead of overwriting.
    """
    path = os.path.abspath(path)
    if os.path.exists(path) and not overwrite:
        raise FileExistsError(
            f"save_synthetic_to_h5: file {path} already exists. "
...
def load_synthetic_from_h5(path, expect_dim=None):
    """
    Load synthetic dataset from an HDF5 file with keys:
    ...
    path : str
        Source HDF5 file.
    expect_dim : int or None
        If not None, assert that data has feature dimension D == expect_dim.
    ...
    data : torch.Tensor
        Tensor of shape [N, D].
    labels : torch.Tensor or None
        Tensor of shape [N, ...] if 'label' dataset exists, else None.
    """
    path = os.path.abspath(path)
    if not os.path.exists(path):
        raise FileNotFoundError(f"load_synthetic_from_h5: file {path} does not exist.")

    with h5py.File(path, "r") as f:
        if "data" not in f:
            raise KeyError("load_synthetic_from_h5: dataset 'data' not found in the HDF5 file.")
        data_np = f["data"][:]
    ...
    if expect_dim is not None and data.shape[1] != expect_dim:
        raise ValueError(
            f"load_synthetic_from_h5: expected feature dimension {expect_dim}, "
