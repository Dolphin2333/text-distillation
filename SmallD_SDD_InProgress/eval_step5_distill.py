...
def load_mrpc_val(data_root: str):
...
def main():
    ...
    parser.add_argument(
        ...
        help="Folder with mrpc_val_emb.pt/mrpc_val_labels.pt",
    )
    parser.add_argument(
        "--paths",
        nargs="+",
    ...
    x_val, y_val = load_mrpc_val(args.data_root)
    ...
    for path in paths:
        ...
        if ipc is None:
            # if not parseable, infer from length and num_classes=2
            num_syn = syn_X.size(0)
            ipc = num_syn // 2
            print(f"  [WARN] IPC not parsed from name; inferred ipc={ipc} from N={num_syn}.")

        # build synthetic labels assuming [class0 block, class1 block]
        num_classes = 2
        syn_y = torch.arange(num_classes).repeat_interleave(ipc)[: syn_X.size(0)]
