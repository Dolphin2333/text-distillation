...
# Force scaled dot-product attention to use the math backend (has full backward support)
try:
    ...
    print(f"Could not configure torch SDPA kernels (this is usually fine on CPU/MPS): {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='mrpc_emb', type=str, help='Dataset to use')
    ...
    parser.add_argument('--inner_lr', default=0.001, type=float, help='inner learning rate')
    parser.add_argument('--label_lr_scale', default=1, type=float, help='scale the label lr')
    parser.add_argument('--num_per_class', default=5, type=int, help='Number of samples per class (IPC)')
    parser.add_argument('--batch_per_class', default=5, type=int, help='Number of samples per class per batch')
    parser.add_argument('--task_sampler_nc', default=4, type=int, help='Number of tasks to sample per batch')
    ...
    parser.add_argument('--num_train_eval', default=4, type=int, help='Num of training of network for evaluation')
    ...
    parser.add_argument('--fname', default='mrpc_mlp_ipc10', type=str, help='Filename for storing checkpoints')
    parser.add_argument('--name', default='mrpc_step1', type=str, help='name of the experiment for wandb')
    ...
    parser.add_argument('--syn_strategy', default='none', type=str, help='Synthetic data augmentation strategy')
    parser.add_argument('--real_strategy', default='none', type=str, help='Real data augmentation strategy')
    ...
    # Boost-DD / warm-started distillation options
    parser.add_argument(
        '--boost_dd',
        action='store_true',
        help='Use Boost-DD style warm-start from a previous synthetic dataset'
    )
    parser.add_argument(
        '--boost_init_from',
        default='none',
        type=str,
        help='Path to an HDF5 file containing previous synthetic data for Boost-DD warm-start (key: "data", optionally "label")'
    )
    parser.add_argument(
        '--boost_beta',
        default=1.0,
        type=float,
        help='Stale meta-learning rate factor for previous synthetic blocks (0=frozen, 1=no boost effect)'
    )
    parser.add_argument(
        '--stage',
        default=0,
        type=int,
        help='Boost-DD stage index (0 for initial block)'
    )
    parser.add_argument(
        '--width',
        default=256,
        type=int,
        help='Model width / hidden size for text models (TextMLP, TextTransformer).'
    )
...
"""
python main.py \
  --root /scratch/zz3645/nlp_project/dataset \
  --dataset mrpc_emb \
  --arch text_mlp \
  --width 256 \
  --num_per_class 10 \
