...
def get_arch(arch, num_classes, channel, im_size, width=64):
    ...
    if arch == 'text_transformer':
        # Same flattening convention as text_mlp; d_in = d_emb for MRPC embeddings
        d_in = channel * im_size[0] * im_size[1]

        # Interpret `width` as the Transformer d_model.
        d_model = width
        # Reasonable defaults; can be tuned later via code changes if needed
        nhead = 4
        num_layers = 2
        seq_len = 4   # ensure d_in is divisible by 4 for MRPC embeddings
...
def get_dataset(dataset, root, transform_train, transform_test, zca=False):
    ...
    if dataset == 'cifar10':
        ...
        # Here, root is already args.data_root, i.e. "<repo>/scripts/mrpc_emb"
        ...
        testset = trainset_test  # for now, use val as test; replace later if you add true test

        num_classes = int(torch.max(trainset.y).item()) + 1
        d_emb = trainset.d_emb

        # Make a "fake image" shape such that channel * H * W = d_emb
        # For embeddings: channel = 1, height = d_emb, width = 1
        shape = [1, d_emb, 1]
        process_config = None
