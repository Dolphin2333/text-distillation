...
def load_data(data_root: str, train_emb: str, train_lbl: str, val_emb: str, val_lbl: str):
    X_train = torch.load(os.path.join(data_root, train_emb), map_location="cpu").float()
    y_train = torch.load(os.path.join(data_root, train_lbl), map_location="cpu").long()
    X_val = torch.load(os.path.join(data_root, val_emb), map_location="cpu").float()
    y_val = torch.load(os.path.join(data_root, val_lbl), map_location="cpu").long()
...
def main():
    ...
    parser.add_argument("--train_emb_file", type=str, default="mrpc_train_emb.pt")
    parser.add_argument("--train_label_file", type=str, default="mrpc_train_labels.pt")
