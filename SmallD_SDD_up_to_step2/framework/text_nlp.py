...
class TextMLP(nn.Module):
    def __init__(self, d_in, num_classes, d_hidden=512):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(d_in, d_hidden),        # layer 1: 768 -> 512
            nn.ReLU(),
            nn.Linear(d_hidden, d_hidden),    # layer 2: 512 -> 512
            nn.ReLU(),
            nn.Linear(d_hidden, d_hidden // 2),  # layer 3: 512 -> 256
            nn.ReLU(),
            nn.Linear(d_hidden // 2, d_hidden // 4),  # layer 4: 256 -> 128
            nn.ReLU(),
            nn.Linear(d_hidden // 4, num_classes),    # output: 128 -> 2
        )
