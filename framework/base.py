...
import random
import math
from framework.config import get_arch, get_dataset, get_transform
from framework.distill_higher import Distill
from framework.util import accuracy


# ===============================
# No-op data intervention
# ===============================
def set_up_interventions(args):
    class NoOp:
        def __init__(self):
            self.not_single = False
            self.keys = []
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    trainset, _, testset, num_classes, shape, _ = get_dataset(args.dataset, args.root, transform_train, transform_test)

    print(f"Loaded dataset: {len(trainset)} train / {len(testset)} test / num_classes={num_classes}")
    ...
    x_init = torch.randn(args.num_per_class * num_classes, flat_dim)
    y_init = torch.arange(num_classes).repeat_interleave(args.num_per_class)
    if args.train_y:
        y_init = nn.functional.one_hot(y_init, num_classes)

    syn_intervention, real_intervention, _ = set_up_interventions(args)

    # ----- 初始化 Distill 模块 -----
    print("==> Building Distill model")
    model = Distill(
        x_init, y_init, args.arch, args.window, args.inner_lr, args.num_train_eval,
        img_pc=args.num_per_class, batch_pc=args.batch_per_class,
        num_classes=num_classes, train_y=args.train_y,
        syn_intervention=syn_intervention, real_intervention=real_intervention,
        device=device,
    ).to(device)

    model.data.weight.data = project(model.data.weight.data)
    print(model.net)

    optimizer = optim.Adam([model.data.weight], lr=args.lr)
    ...
    for epoch in range(args.epochs):
        train_loss, train_acc = train(train_loader, model, optimizer, criterion, args, epoch)
        test_acc = evaluate(test_loader, model, criterion, args)
...
def train(train_loader, model, optimizer, criterion, args, epoch):
    """
    🧠 NLP Dataset Distillation — Training Loop
    ----------------------------------------------------------
    主要作用：
      - 用合成 embedding 优化 Distill 模块；
      - 在 tqdm 下实时显示 loss / acc；
      - 兼容单 GPU / CPU。
    """
    ...
    for step, (inputs, labels) in enumerate(progress_bar):
        ...
        # 🔁 前向 + 反向传播
        outputs, _ = model(inputs)  # 这里 Distill.forward 已经内部做了 higher 内环
...
#         # ====================== 初始化蒸馏数据（支持多模态） ======================
#     # shape 从 get_dataset 返回，可以是：
#     #   - 图像: (C, H, W)
#     #   - 文本: (L, D) 或 (D,)
#     shape = tuple(shape)
#     print('Input shape from dataset:', shape)

#     if len(shape) == 3:
#         # 图像数据: (C, H, W)
#         channel, height, width = shape
#     elif len(shape) == 2:
#         # 文本等: (L, D)，这里虚拟一个 channel=1，当成 (1, L, D)
#         channel = 1
#         height, width = shape
#     elif len(shape) == 1:
#         # 已经是扁平向量: (feature_dim,)
#         channel = 1
#         height, width = shape[0], 1
#     else:
#         raise ValueError(f"Unsupported input shape: {shape}")

#     flat_dim = channel * height * width
#     print('Flattened input dim for distilled data:', flat_dim)

#     # 初始化合成数据：随机高维向量，后续会被 Distill 学成“合成样本”
#     x_init = torch.randn(args.num_per_class * num_classes, flat_dim)
...
#     model = Distill(
#         x_init, y_init, args.arch, args.window, args.inner_lr, args.num_train_eval,
#         img_pc=args.num_per_class, batch_pc=args.batch_per_class, train_y=args.train_y,
#         channel=channel,                              # 不再直接用 shape[0]
#         num_classes=num_classes, task_sampler_nc=args.task_sampler_nc,
#         im_size=(height, width),                      # 不再假设 shape[1], shape[2] 一定存在
...
# def set_up_interventions(args):
#     """
#     根据任务类型设置合成数据和真实数据的“干预/增强”策略。
#     图像任务：使用 ImageIntervention 做数据增强；
#     文本任务：返回 no-op，对输入不做任何修改。
#     """
#     # 如果是常见的 NLP 文本数据集，直接返回 no-op
#     if args.dataset.lower() in ['agnews', 'imdb', 'yelp', 'sst2']:
#         class NoOpIntervention:
#             def __init__(self):
#                 self.not_single = False  # 为了兼容打印
#                 self.keys = []           # 没有任何增强 key

#             def __call__(self, x):
#                 # 不做任何改变，直接返回原样
#                 return x

#         syn_intervention = NoOpIntervention()
#         real_intervention = NoOpIntervention()
#         intervention_prob = 1.0
#         return syn_intervention, real_intervention, intervention_prob

#     # ===== 图像任务：沿用原来的 ImageIntervention 增强 =====
#     syn_intervention  = ImageIntervention(
#                              'syn_aug',
#                              args.syn_strategy,
#                              phase='test',
